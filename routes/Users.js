...
router.put("/changepassword", validateToken, async (req, res) => {
  ...
  const user = await Users.findOne({
    where: { username: req.user.username /* uzimamo username preko tokena */ },
  });
  ...
  //lista korisnika
  router.get("/listofusers", validateToken, async (req, res) => {
    try {
      // Dohvatanje svih korisnika iz tabele Users
      const users = await Users.findAll();

      // Slanje korisnika kao JSON odgovor
      res.json({ users: users });
    } catch (error) {
      console.error("Greška pri dobijanju liste korisnika:", error);
      res.status(500).json({ error: "Greška pri dobijanju liste korisnika" });
    }
  });
});
